...
define(
		...
		function(Dialog, PageBase) {
			var AfaUserUpDlg = PageBase.extend({
						// 类初始化
						initialize : function() {
							AfaUserUpDlg.superclass.initialize.call(this);
						},
						ACTION : {
							ADD : "ADD",
							EDIT : "EDIT",
						...
						listeners : {
                            ...
                            afaUserPage_saveaddBtn : {
								click : function() {
									var self = AfaUserUpDlg.getInstance();
									self.doSave({
										isSaveClose : true
									});
						...
						doSave : function(config, callback) {
                            ...
                            if($A("#startDate").datetime("getValue")>$A("#endDate").datetime("getValue")){
								...
								if(!AfaUserUpDlg.getInstance().validateUserCode(userCode)){
						...
						showPage : function(action, data, callback) {
							...
							AfaUserUpDlg.getInstance().orgMsg=data;
							...
							var options = {
								...
								hasheader : false,
								reload : true,
								onPageLoad : this.showPageCallBack
							};
			...
			// 创建窗体类单例
			AfaUserUpDlg.getInstance = function() {
				if (!this.instance) {
					this.instance = new AfaUserUpDlg();
				}
				return this.instance;
			}

			return AfaUserUpDlg.getInstance();
		})