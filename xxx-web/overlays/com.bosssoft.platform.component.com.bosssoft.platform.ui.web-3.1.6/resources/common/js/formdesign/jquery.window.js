	...
	function create(target){
		...
		var win = $(target).panel($.extend({}, state.options, {
			...
			onOpen: function(){
				...
				state.window.css('z-index', Options.zindexs.dialog++);
	...
	function setProperties(target){
		...
		state.window.draggable({
			...
			helper: function( event ) {
       				 return $('<div class="window-proxy"></div>');
      			},
			//disabled: state.options.draggable == false,
			start: function(e,dragobj){
		
				if (state.mask) state.mask.css('z-index', Options.zindexs.dialog++);
				if (state.shadow) state.shadow.css('z-index', Options.zindexs.dialog++);
				state.window.css('z-index', Options.zindexs.dialog++);
				
				if (!state.proxy){
					state.proxy =dragobj.helper;
				}
				state.proxy.css({
					display:'none',
					zIndex: Options.zindexs.dialog++,
			...
			stop: function(e,dragobj){
				state.options.left =dragobj.position.left;
			state.options.top = dragobj.position.top;
				$(target).window('move');
				state.proxy.remove();
				state.proxy = null;
			}
		...
		state.window.resizable({
			...
			start:function(e,resizableobj){
				...
				resizableobj.helper.css({	zIndex: Options.zindexs.dialog++});
			},
			resize: function(e,resizableobj){
			/*	state.proxy.css({
					left: e.data.left,
					top: e.data.top
				});
			...
			stop: function(e,resizableobj){
			
				$(target).window('resize',resizableobj.size);
				/*state.pmask.remove();
				state.pmask = null;
				state.proxy.remove();
				state.proxy = null;*/
...
//					$(this).appendTo('body');
	...
	$.fn.window.parseOptions = function(target){
		return $.extend({},Panel.parseOptions(target), $.fn.parseOptions(target, [
			{draggable:'boolean',resizable:'boolean',shadow:'boolean',modal:'boolean',inline:'boolean'}
		]));
	};
	
	// Inherited from $.fn.panel.defaults
	$.fn.window.defaults = $.extend({}, Options.appDefaults.Panel, {
		zIndex: 9000,
		draggable: true,
		resizable: true,
		shadow: true,
