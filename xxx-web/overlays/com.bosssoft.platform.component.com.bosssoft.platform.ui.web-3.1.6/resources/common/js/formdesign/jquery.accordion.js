		...
		pp.panel($.extend({}, {
			...
			onBeforeExpand: function(target){
				if (options.onBeforeExpand){
					if (options.onBeforeExpand.call(target) == false){return false}
				}
				if (!opts.multiple){
			...
			onExpand: function(target){
				if (options.onExpand){options.onExpand.call(target)}
				opts.onSelect.call(container, $(target).panel('options').title, getPanelIndex(container, target));
			},
			onBeforeCollapse: function(target){
				if (options.onBeforeCollapse){
					if (options.onBeforeCollapse.call(target) == false){return false}
				}
			...
			onCollapse: function(target){
				if (options.onCollapse){options.onCollapse.call(target)}
				opts.onUnselect.call(container, $(target).panel('options').title, getPanelIndex(container, target));
			}
		}));
